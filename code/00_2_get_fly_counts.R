...
files_folders = grep("design", list.files(experiment_folder, full.names = T), value = T, ignore.case = T)
...
all_counts = lapply(1:length(files_folders), function(i){
  ...
  # design_exp_i_left = design_exp_i_2 %>% 
  #   select(ends_with("_left")) %>% 
  #   rowwise() %>% 
  #   mutate(sum_cols = sum(c_across(ends_with("_left")))) %>% 
  #   mutate_at(vars(ends_with("_left")), ~./sum_cols) %>% 
  #   select(-sum_cols) %>% 
  #   ungroup()
  # 
  # design_exp_i_right = design_exp_i_2 %>% 
  #   select(ends_with("_right")) %>% 
  #   rowwise() %>% 
  #   mutate(sum_cols = sum(c_across(ends_with("_right")))) %>% 
  #   mutate_at(vars(ends_with("_right")), ~./sum_cols) %>% 
  #   select(-sum_cols) %>% 
  #   ungroup()
  # 
  # design_exp_i_scaled = design_exp_i_left %>% 
  #   bind_cols(design_exp_i_right) %>% 
  #   mutate(key = keys)
...
write_csv(all_counts, here(paste0("data/counts_", date_char, ".csv")))
