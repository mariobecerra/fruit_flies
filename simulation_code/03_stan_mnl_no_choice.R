...
design_array_i_opt_01_opdesmixr = opdesmixr::mnl_mixture_coord_exch(
  ...
  n_random_starts = 1, n_cores = 1, max_it = 5)


# Repeat with 20 respondents
design_array_i_opt_01_df = lapply(1:20, function(r){
  print(r)
...
simulated_data_i_opt_01 = simulate_mixture_choice_data_no_choice(
  design_array = design_array_i_opt_01, 
  beta_vector_mixture = beta_vec_01_mixture,
  beta_no_choice = beta_no_choice,
  order = 1, 
...
sim_data_mlogit_i_opt_01 <- dfidx(
  simulated_data_i_opt_01 %>% 
    select(starts_with("comp"), choice_set, choice, no_choice_ind), 
  idx = c("choice_set"))
...
p1 <- stan(model_code = m1, data = stan_data_01, iter = 1000, chains = 4, seed = 2023, cores = 4)
p1
