  ...
  out_exp_e = lapply(1:S, function(s){
    ...
    out_s = lapply(1:n_images_per_choice_set, function(im){
      ...
      sim_data_im = simulate_mixture_choice_data_no_choice_many_respondents_with_covariates(
        ...
        beta_vector_mixture = beta_level_1[1:(q-1)],
        beta_no_choice = beta_level_1[q],
...
saveRDS(simulated_data, paste0(sims_out_folder, "model_1_level_sim_simulated_data_tibble_", date_today, ".rds"))
...
# 1 minute for 1500 iterations and 6 experiments, 50 choice sets and 5 images per choice set
model_stan <- stan(
  file = here("simulation_code/11_hmnl_flies_1_level_w_extra_vars.stan"),
  data = stan_data, 
  seed = 2023,
...
saveRDS(model_stan, paste0(sims_out_folder, "model_1_level_sim_stan_object_", date_today, ".rds"))
...
# plot(model_stan, plotfun="trace", pars=("beta_level_2"))


matrix(as.data.frame(summary(model_stan, pars = c("Sigma_level_0"), probs = c(0.5))$summary)$`50%`, 
       ncol = stan_data$n_mixture_cols+1)

matrix(as.data.frame(summary(model_stan, pars = c("Sigma_level_1"), probs = c(0.5))$summary)$`50%`, 
       ncol = stan_data$n_mixture_cols+1)
