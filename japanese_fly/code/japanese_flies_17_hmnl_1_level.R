...
unique_experiment_choice_set_image = counts_japanese_sample %>%
  select(experiment, folder, image, choice_set) %>%
  distinct()
...
index_dataframe_image = lapply(1:nrow(unique_experiment_choice_set_image), function(i){
  ...
  folder_i =  unique_experiment_choice_set_image$folder[i]
  cs_i =  unique_experiment_choice_set_image$choice_set[i]
  image_i =  unique_experiment_choice_set_image$image[i]
  
  # Inefficient way to do this, but too lazy to optimize it
  indices_i = which(counts_japanese_sample$choice_set == cs_i & counts_japanese_sample$experiment == exp_i & counts_japanese_sample$image == image_i & counts_japanese_sample$folder == folder_i)
...
# With 9 experiments and 30 images per choice set:
# Gradient evaluation took 0.071397 seconds
# 1000 transitions using 10 leapfrog steps per transition would take 713.97 seconds.
# 16 hours for 800 iterations (300 warmup)
# No warnings about treedepth or divergent transitions
...
if(file.exists(model_01_filename)){
  ...
  model_stan_01 <- stan(
    ...
    iter = 800,  warmup = 300, chains = 4, cores = 4,
    init = init_fun
  )
  print(Sys.time())
